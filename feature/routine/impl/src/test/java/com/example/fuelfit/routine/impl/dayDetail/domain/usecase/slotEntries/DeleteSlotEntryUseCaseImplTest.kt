...
class DeleteSlotEntryUseCaseImplTest {

    @MockK
    lateinit var repository: DayDetailRepository

    private lateinit var useCase: DeleteSlotEntryUseCaseImpl

    @Before
    fun setUp() {
        MockKAnnotations.init(this)
        useCase = DeleteSlotEntryUseCaseImpl(repository)
    }

    @Test
    fun `when repository returns Success then use case returns Unit`() = runTest {
        coEvery { repository.deleteSlotEntry(1) } returns ResultWrapper.Success(Unit)

        val result = useCase.invoke(1)

        assertTrue(result is ResultWrapper.Success)
        coVerify(exactly = 1) { repository.deleteSlotEntry(1) }
    }

    @Test
    fun `when repository returns Error then use case returns same Error`() = runTest {
        val error = ApiError(404, "Not Found")
        coEvery { repository.deleteSlotEntry(99) } returns ResultWrapper.Error(error)

        val result = useCase.invoke(99)

        assertTrue(result is ResultWrapper.Error)
        coVerify(exactly = 1) { repository.deleteSlotEntry(99) }
    }

    @Test
    fun `when id is invalid then repository returns Error`() = runTest {
        val error = ApiError(400, "Invalid ID")
        coEvery { repository.deleteSlotEntry(-1) } returns ResultWrapper.Error(error)
